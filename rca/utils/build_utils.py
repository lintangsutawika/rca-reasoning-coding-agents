...
from rca.utils.args_parser import get_parser
from rca.utils.constants import EVAL_AGENT_SERVER_IMAGE
from rca.utils.image_utils import image_exists
...
def _get_sdk_submodule_info() -> tuple[str, str, str]:
    ...
    try:
        result = subprocess.run(
            ...
            ["git", "submodule", "status", "software-agent-sdk"],
            cwd=benchmarks_root,
            capture_output=True,
            text=True,
            check=True,
    ...
    sdk_path = benchmarks_root / "software-agent-sdk"
    try:
        result = subprocess.run(
